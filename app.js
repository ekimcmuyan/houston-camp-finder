...
(function () {
  ...
  function migrateProfile(profile) {
    ...
    if (!profile.summerStart || !profile.summerEnd) {
      ...
      if (d && d.lastDay) {
        ...
        profile.summerStart = "2026-06-04";
        profile.summerEnd = "2026-08-24";
      }
    }
  ...
  function initApp() {
    ...
    subtitle.textContent = kidNames ? `Summer camps for ${kidNames}` : "Summer Camps for the Houston Area";
  ...
  function renderCategoryTabs() {
    ...
    html += CATEGORIES.map(cat => {
      ...
      const bg = cat.id === currentCategory ? `background:${cat.color};border-color:${cat.color};box-shadow:0 0 12px ${cat.color}40;` : "";
